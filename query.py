...
SYSTEM_PROMPT = """You are a helpful research assistant with access to a library of blog articles.
Answer the user's question using only the context provided below.
Where relevant, reference the specific articles the information came from.
...
def ask(question):
    ...
    for i, (chunk, metadata, distance) in enumerate(zip(chunks, metadatas, distances)):
        print(f"\n  [{i+1}] {metadata.get('title', 'N/A')} | Distance: {round(distance, 4)}")
        print(f"       {chunk[:150]}...")
    ...
    # Print source URLs so the user can read the full articles
    seen_urls = []
    print(f"\nSources:")
...
if __name__ == "__main__":
    print("Client Blog RAG — type your question or 'quit' to exit.")
    print("=" * 60)
